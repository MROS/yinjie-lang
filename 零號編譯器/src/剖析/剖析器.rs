...
fn 剖析破(詞組: &[Ｏ詞]) -> Option<(Ｏ破, &[Ｏ詞])> {
    消耗(破)(詞組).映射(|_| Ｏ破 {})
}

fn 剖析續(詞組: &[Ｏ詞]) -> Option<(Ｏ續, &[Ｏ詞])> {
    消耗(續)(詞組).映射(|_| Ｏ續 {})
}
...
fn 剖析賦值(詞組: &[Ｏ詞]) -> Option<(Ｏ賦值, &[Ｏ詞])> {
    剖析識別子(詞組)
        .且棄(消耗(賦值))
        .且(剖析算式)
        .映射(|(變數名, 算式)| {Ｏ賦值{算式, 變數名}})
}
