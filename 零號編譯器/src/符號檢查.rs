use rpds::HashTrieMap;

use crate::分詞器::Ｏ運算子;
use crate::剖析::抽象語法樹節點::*;
use crate::外術列表::外術列表;

#[derive(Clone)]
enum Ｏ類型 {
    變數,
    術 {
        // TODO: 未來要讓回傳值也是函式簽名的一部分
        形參數量: usize,
    },
}

#[derive(Clone)]
struct Ｏ環境 {
    符號表: HashTrieMap<String, Ｏ類型>,
    環層級: usize,
}

impl Ｏ環境 {
    fn new() -> Self {
        Ｏ環境 {
            符號表: HashTrieMap::new(),
            環層級: 0,
        }
    }
}

pub fn 檢查語法樹(語法樹: &Ｏ語法樹) -> bool {
    let mut 環境 = Ｏ環境::new();

    for (術名, 形參數量) in 外術列表() {
        環境.符號表.insert_mut(術名, Ｏ類型::術 { 形參數量 });
    }

    let mut 存在初術 = false;

    let mut 通過 = true;
    for 頂層宣告 in &語法樹.頂層宣告 {
        通過 = match 頂層宣告 {
            Ｏ頂層宣告::術宣告(宣告) => {
                if 宣告.術名 == "初" {
                    存在初術 = true;
                }
                檢查術宣告(&mut 環境, 宣告)
            }
            Ｏ頂層宣告::變數宣告(宣告) => 檢查變數宣告(&mut 環境, 宣告),
        } && 通過;
    }

    if !存在初術 {
        panic!("必須定義音界咒入口「初」術")
    }

    通過
}

fn 檢查術宣告(環境: &mut Ｏ環境, 術宣告: &Ｏ術宣告) -> bool {
    // 將術名加進原環境中
    環境.符號表.insert_mut(
        術宣告.術名.clone(),
        Ｏ類型::術 {
            形參數量: 術宣告.形參.len(),
        },
    );

    // 先加術名再複製出新環境，如此便可允許遞迴
    let mut 新環境 = 環境.clone();

    // 檢查形參並將其加入新的環境
    for 形參 in &術宣告.形參 {
        新環境.符號表.insert_mut(形參.clone(), Ｏ類型::變數);
    }

    // 檢查術體的每一句
    let 通過 = 檢查區塊(新環境, &術宣告.術體);

    通過
}

fn 檢查句(mut 環境: &mut Ｏ環境, 句: &Ｏ句) -> bool {
    match 句 {
        Ｏ句::變數宣告(變數宣告) => 檢查變數宣告(&mut 環境, 變數宣告),
        Ｏ句::算式(算式) => 檢查算式(環境, 算式),
        Ｏ句::歸(歸) => 檢查算式(環境, &歸.0),
        Ｏ句::若(若) => 檢查若(環境, &若),
        Ｏ句::環(環) => 檢查環(環境,環),
        Ｏ句::破(_) => 檢查破(環境),
        Ｏ句::續(_) => 檢查續(環境),
        Ｏ句::賦值(賦值) => 檢查賦值(環境, &賦值),
    }
}

fn 檢查若(環境: &Ｏ環境, 若: &Ｏ若) -> bool {
    let 條件通過 = 檢查算式(環境, &若.條件);

    let 區塊通過 = 檢查區塊(環境.clone(), &若.區塊);

    let mut 或若通過 = true;
    for 或若 in &若.或若列表 {
        let 條件通過 = 檢查算式(環境, &或若.條件);
        let 或若區塊通過 = 檢查區塊(環境.clone(), &或若.區塊);
        或若通過 = 或若通過 && 條件通過 && 或若區塊通過;
    }

    let 不然通過 = match &若.不然 {
        Some(不然區塊) => 檢查區塊(環境.clone(), &不然區塊.區塊),
        None => true,
    };

    條件通過 && 區塊通過 && 或若通過 && 不然通過
}

fn 檢查環(環境: &Ｏ環境, 環: &Ｏ環) -> bool {
    let mut 新環境 = 環境.clone();
    新環境.環層級 += 1;
    檢查區塊(新環境, &環.區塊)
}

fn 在環內(環境: &Ｏ環境) -> bool {
    環境.環層級 > 0
}

fn 檢查續(環境: &Ｏ環境) -> bool {
    在環內(環境)
}

fn 檢查破(環境: &Ｏ環境) -> bool {
    在環內(環境)
}

fn 檢查變數宣告(環境: &mut Ｏ環境, 變數宣告: &Ｏ變數宣告) -> bool {
    let 通過 = 檢查算式(環境, &變數宣告.算式);
    環境.符號表.insert_mut(變數宣告.變數名.clone(), Ｏ類型::變數);

    通過
}

fn 檢查賦值(環境: &mut Ｏ環境, 賦值: &Ｏ賦值) -> bool {
    let 變數已宣告 = 環境.符號表.contains_key(&賦值.變數名);
    let 算式通過 = 檢查算式(環境, &賦值.算式);

    變數已宣告 && 算式通過
}

// 不會增加定義（改變環境）的語法結構返回檢查是否成功即可
fn 檢查算式(環境: &Ｏ環境, 算式: &Ｏ算式) -> bool {
    match 算式 {
        Ｏ算式::變數(變數名) => {
            if 環境.符號表.contains_key(變數名) {
                // println!("找到「{}」", 變數名);
                true
            } else {
                println!("「{}」未宣告", 變數名);
                false
            }
        }
        Ｏ算式::數字(_) => true,
        Ｏ算式::二元運算(二元運算) => {
            let 左通過 = 檢查算式(環境, 二元運算.左.as_ref());
            let 右通過 = 檢查算式(環境, 二元運算.右.as_ref());
            左通過 && 右通過
        }
        Ｏ算式::施術(施術) => match 環境.符號表.get(&施術.術名) {
            Some(Ｏ類型::術 { 形參數量 }) => {
                if *形參數量 == 施術.實參.len() {
                    true
                } else {
                    println!("「{}」形參實參數量不同", 施術.術名);
                    false
                }
            }
            Some(Ｏ類型::變數) => {
                println!("「{}」為一變數，不可施展", 施術.術名);
                false
            }
            None => {
                println!("環境中找不到術「{}」", 施術.術名);
                println!("術需先定義後施展");
                false
            }
        },
    }
}

fn 檢查區塊(mut 環境: Ｏ環境, 句組: &[Ｏ句]) -> bool {
    let mut 通過 = true;

    for 句 in 句組 {
        通過 = 檢查句(&mut 環境, 句) && 通過;
    }

    通過
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn 測試變數宣告_數字() {
        let 變數宣告 = Ｏ變數宣告 {
            變數名: "x".to_owned(),
            算式: Ｏ算式::數字(42),
        };

        let mut 環境 = Ｏ環境::new();
        let 通過 = 檢查變數宣告(&mut 環境, &變數宣告);

        assert!(通過);
        assert!(環境.符號表.contains_key("x"));
    }

    #[test]
    fn 測試變數宣告_變數未宣告就使用() {
        let 變數宣告 = Ｏ變數宣告 {
            變數名: "x".to_owned(),
            算式: Ｏ算式::變數("y".to_owned()),
        };

        let mut 環境 = Ｏ環境::new();
        let 通過 = 檢查變數宣告(&mut 環境, &變數宣告);

        assert!(!通過);
    }

    #[test]
    fn 測試術宣告_將術加入環境() {
        let 術宣告 = Ｏ術宣告 {
            術名: "和".to_owned(),
            形參: vec!["甲".to_owned(), "乙".to_owned()],
            術體: vec![Ｏ句::歸(Ｏ歸(Ｏ算式::二元運算(Ｏ二元運算 {
                運算子: Ｏ運算子::加,
                左: Box::new(Ｏ算式::變數("甲".to_owned())),
                右: Box::new(Ｏ算式::變數("乙".to_owned())),
            })))],
        };

        let mut 環境 = Ｏ環境::new();
        let 通過 = 檢查術宣告(&mut 環境, &術宣告);

        assert!(通過);
        assert!(環境.符號表.contains_key("和"));
        match 環境.符號表.get("和") {
            Some(Ｏ類型::術 { 形參數量 }) => assert_eq!(*形參數量, 2),
            _ => panic!("「和」術未正確宣告"),
        }
    }

    #[test]
    fn 測試語法樹_術宣告與施術_形參實參數量一致() {
        // 建立語法樹，包含術宣告與施術
        let 語法樹 = Ｏ語法樹 {
            頂層宣告: vec![
                // 宣告初術
                Ｏ頂層宣告::術宣告(Ｏ術宣告 {
                    術名: "初".to_owned(),
                    形參: vec![],
                    術體: vec![],
                }),
                // 宣告術
                Ｏ頂層宣告::術宣告(Ｏ術宣告 {
                    術名: "和".to_owned(),
                    形參: vec!["甲".to_owned(), "乙".to_owned()],
                    術體: vec![Ｏ句::歸(Ｏ歸(Ｏ算式::二元運算(Ｏ二元運算 {
                        運算子: Ｏ運算子::加,
                        左: Box::new(Ｏ算式::變數("甲".to_owned())),
                        右: Box::new(Ｏ算式::變數("乙".to_owned())),
                    })))],
                }),
                // 變數宣告，施術包含在裡面
                Ｏ頂層宣告::變數宣告(Ｏ變數宣告 {
                    變數名: "結果".to_owned(),
                    算式: Ｏ算式::施術(Ｏ施術 {
                        術名: "和".to_owned(),
                        實參: vec![Ｏ算式::數字(10), Ｏ算式::數字(20)],
                    }),
                }),
            ],
        };

        // 檢查語法樹
        let 檢查結果 = 檢查語法樹(&語法樹);

        // 檢查結果
        assert!(檢查結果); // 應該通過
    }

    #[test]
    fn 測試語法樹_術宣告與施術_形參實參數量不一致() {
        // 建立語法樹，包含術宣告與施術
        let 語法樹 = Ｏ語法樹 {
            頂層宣告: vec![
                // 宣告初術
                Ｏ頂層宣告::術宣告(Ｏ術宣告 {
                    術名: "初".to_owned(),
                    形參: vec![],
                    術體: vec![],
                }),
                // 宣告術
                Ｏ頂層宣告::術宣告(Ｏ術宣告 {
                    術名: "和".to_owned(),
                    形參: vec!["甲".to_owned(), "乙".to_owned()],
                    術體: vec![Ｏ句::歸(Ｏ歸(Ｏ算式::二元運算(Ｏ二元運算 {
                        運算子: Ｏ運算子::加,
                        左: Box::new(Ｏ算式::變數("甲".to_owned())),
                        右: Box::new(Ｏ算式::變數("乙".to_owned())),
                    })))],
                }),
                // 變數宣告，施術包含在裡面
                Ｏ頂層宣告::變數宣告(Ｏ變數宣告 {
                    變數名: "結果".to_owned(),
                    算式: Ｏ算式::施術(Ｏ施術 {
                        術名: "和".to_owned(),
                        實參: vec![
                            Ｏ算式::數字(10), // 只提供一個實參
                        ],
                    }),
                }),
            ],
        };

        // 檢查語法樹
        let 檢查結果 = 檢查語法樹(&語法樹);

        // 檢查結果
        assert!(!檢查結果); // 應該失敗
    }

    #[test]
    fn 測試若語句_條件正確() {
        let 若語句 = Ｏ若 {
            條件: Ｏ算式::變數("甲".to_owned()),
            區塊: vec![Ｏ句::算式(Ｏ算式::數字(1))],
            或若列表: vec![],
            不然: None,
        };

        let mut 環境 = Ｏ環境::new();
        環境.符號表.insert_mut("甲".to_owned(), Ｏ類型::變數);

        let 通過 = 檢查若(&環境, &若語句);

        assert!(通過);
    }

    #[test]
    fn 測試若語句_條件未定義() {
        let 若語句 = Ｏ若 {
            條件: Ｏ算式::變數("甲".to_string()),
            區塊: vec![Ｏ句::算式(Ｏ算式::數字(1))],
            或若列表: vec![],
            不然: None,
        };

        let 環境 = Ｏ環境::new(); // 「甲」未宣告

        let 通過 = 檢查若(&環境, &若語句);

        assert!(!通過);
    }

    #[test]
    fn 測試句組_多個句子() {
        let 句組 = vec![
            Ｏ句::變數宣告(Ｏ變數宣告 {
                變數名: "甲".to_string(),
                算式: Ｏ算式::數字(1),
            }),
            Ｏ句::變數宣告(Ｏ變數宣告 {
                變數名: "乙".to_string(),
                算式: Ｏ算式::二元運算(Ｏ二元運算 {
                    運算子: Ｏ運算子::加,
                    左: Box::new(Ｏ算式::變數("甲".to_string())),
                    右: Box::new(Ｏ算式::數字(1)),
                }),
            }),
        ];

        let 環境 = Ｏ環境::new();
        let 通過 = 檢查區塊(環境, &句組);

        assert!(通過);
    }

    #[test]
    fn 測試賦值_變數已宣告() {
        let 賦值 = Ｏ賦值 {
            變數名: "甲".to_owned(),
            算式: Ｏ算式::數字(2),
        };

        let mut 環境 = Ｏ環境::new();
        環境.符號表.insert_mut("甲".to_owned(), Ｏ類型::變數);

        let 通過 = 檢查賦值(&mut 環境, &賦值);

        assert!(通過);
    }

    #[test]
    fn 測試賦值_變數未宣告() {
        let 賦值 = Ｏ賦值 {
            變數名: "甲".to_owned(),
            算式: Ｏ算式::數字(2),
        };

        let mut 環境 = Ｏ環境::new();

        let 通過 = 檢查賦值(&mut 環境, &賦值);

        assert!(!通過);
    }

    #[test]
    fn 測試賦值_右側算式變數未宣告() {
        let 賦值 = Ｏ賦值 {
            變數名: "甲".to_owned(),
            算式: Ｏ算式::變數("乙".to_owned()),
        };

        let mut 環境 = Ｏ環境::new();
        環境.符號表.insert_mut("甲".to_owned(), Ｏ類型::變數);

        let 通過 = 檢查賦值(&mut 環境, &賦值);

        assert!(!通過);
    }

    #[test]
    fn 測試環_包含破和續() {
        let 環語句 = Ｏ環 {
            區塊: vec![Ｏ句::破(Ｏ破 {}), Ｏ句::續(Ｏ續 {})],
        };

        let 環境 = Ｏ環境::new();
        let 通過 = 檢查環(&環境, &環語句);

        assert!(通過);
    }

    #[test]
    fn 測試破_不在環內() {
        let 環境 = Ｏ環境::new();
        let 通過 = 檢查破(&環境);
        assert!(!通過);
    }

    #[test]
    fn 測試續_不在環內() {
        let 環境 = Ｏ環境::new();
        let 通過 = 檢查續(&環境);
        assert!(!通過);
    }
}