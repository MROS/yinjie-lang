...
impl Ｏ真言生成器 {
    ...
    fn 生成句(&mut self, 句: &Ｏ句, 符號表: &mut Ｏ符號表) -> io::Result<()> {
        match 句 {
            Ｏ句::變數宣告(變數宣告) => self.生成變數宣告(變數宣告, 符號表),
            // 算式可能含有副作用，如「曰」會打印數字
            // 不可為了優化直接省略掉
            Ｏ句::算式(算式) => {
                self.計算(算式, &符號表)?;
            ...
            Ｏ句::環(環) => self.生成環(環, 符號表),
            Ｏ句::破(_) => self.生成破(),
            Ｏ句::續(_) => self.生成續(),
            Ｏ句::賦值(賦值) => self.生成賦值(賦值, 符號表),
        }
    }
    ...
    fn 生成賦值(&mut self, 賦值: &Ｏ賦值, 符號表: &mut Ｏ符號表) -> io::Result<()> {
        self.計算(&賦值.算式, 符號表)?;
        self.彈出()?;

        符號表
            .取得變數位址(&賦值.變數名)
            .寫出(&mut self.真言檔, "t0", &賦值.變數名)
    }
    ...
    fn 新若語句群結尾標籤名(&mut self) -> String {
        self.若語句群結尾標籤計數 += 1;
        format!("若語句群結尾標籤——{}", self.若語句群結尾標籤計數)
    }
    ...
    fn 生成環(&mut self, 環: &Ｏ環, 符號表: &mut Ｏ符號表) -> io::Result<()> {
        self.環計數 += 1;
        let 當前環 = self.環計數;
        self.當前環棧.push(當前環);

        let 起點 = format!("環起點——{}", 當前環);
        let 終點 = format!("環終點——{}", 當前環);

        self.上標籤(&起點)?;
        self.生成區塊(&環.區塊, 符號表.clone())?;
        self.跳至標籤(&起點)?;
        self.上標籤(&終點)?;
    ...
    fn 生成破(&mut self) -> io::Result<()> {
        let 當前環 = self.當前環棧.last().unwrap();
        let 終點 = format!("環終點——{}", 當前環);
        self.跳至標籤(&終點)
    }
    fn 生成續(&mut self) -> io::Result<()> {
        let 當前環 = self.當前環棧.last().unwrap();
        let 起點 = format!("環起點——{}", 當前環);
        self.跳至標籤(&起點)
    }
